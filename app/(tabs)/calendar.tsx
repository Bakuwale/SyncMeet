...
export default function CalendarScreen() {
  ...
  // Debug theme changes
  console.log('Calendar theme changed:', { theme, isDarkTheme, calendarBackground: themeColors.calendarBackground, textPrimary: themeColors.textPrimary });

  // Mark today's date and scheduled dates
  ...
  const handleDayPress = (day: { dateString: string }) => {
    ...
    const now = new Date(todayString); // Only date, no time
    if (picked < now) {
  ...
  const handleAddSchedule = async () => {
    ...
    const msUntilAlarm = scheduleDateTime.getTime() - Date.now();
    if (msUntilAlarm > 0) {
      setTimeout(() => {
  ...
  return (
    ...
    >
      <View style={[styles.container, { backgroundColor: themeColors.background }]}>
        <Calendar
          key={theme}
          style={[styles.calendar, { borderBottomColor: themeColors.borderColor }]}
          markingType={'custom'}
        ...
        {selectedDate && (
          <View style={styles.inputSection}>
            <Text style={[styles.selectedText, { color: themeColors.textPrimary }]}>
              Schedule for <Text style={{ color: themeColors.accent }}>{selectedDate}</Text>
            </Text>
            <TextInput
              style={[styles.input, { 
        ...
        {schedulesForDate.length > 0 && (
          <View style={styles.schedulesSection}>
            ...
            <FlatList
              data={schedulesForDate}
              keyExtractor={(item) => item.id}
              renderItem={({ item }) => (
                <View style={[styles.scheduleItem, { backgroundColor: themeColors.inputBackground }]}>
